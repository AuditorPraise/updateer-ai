...
import { Check, Zap, Shield, Rocket, ArrowLeft, ExternalLink, Building, User, Info } from 'lucide-react';
...
const PricingPage: React.FC<PricingPageProps> = ({ onSelectPlan, onBack, currentCredits, currentCampaigns }) => {
  const [tier, setTier] = useState<'personal' | 'agency'>('personal');
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-slate-400 max-w-2xl mx-auto mb-6">
            Flexible pricing for individuals and agencies.
          </p>
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 max-w-xl mx-auto mb-8 flex items-start gap-3 text-left">
            ...
            <p className="text-sm text-blue-200">
              <strong className="text-blue-100">Note:</strong> An active monthly subscription (Pro or Agency) is required to access the app dashboard. Top-up packs are for adding extra credits to your account.
            </p>
          </div>
          ...
          <div className="inline-flex bg-slate-900 p-1 rounded-lg border border-slate-800 relative">
            <button 
        ...
        {tier === 'personal' ? (
          ...
          <div className="flex justify-center mb-16 animate-fadeIn max-w-4xl mx-auto">
            {/* Mega Top-Up */}
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 hover:border-slate-600 transition-all duration-300 w-full max-w-md">
              <h3 className="text-2xl font-bold mb-2">{MEGA_TOPUP.name}</h3>
              <p className="text-slate-400 text-sm mb-6">{MEGA_TOPUP.description}</p>
              <div className="flex items-baseline gap-1 mb-8">
                <span className="text-4xl font-bold">${MEGA_TOPUP.price}</span>
                <span className="text-slate-500">/pack</span>
              </div>
              ...
              <ul className="space-y-4 mb-8">
                <li className="flex items-start gap-3">
                  ...
                  <span className="text-slate-300">+{MEGA_TOPUP.credits.toLocaleString()} Credits</span>
                </li>
                <li className="flex items-start gap-3">
                  <Rocket className="w-5 h-5 text-cyan-400 shrink-0 mt-0.5" />
                  <span className="text-slate-300">+{MEGA_TOPUP.campaigns} Campaigns</span>
                </li>
                <li className="flex items-start gap-3">
              ...
              <button 
                onClick={() => handlePurchase(MEGA_TOPUP.id)}
                className="w-full py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold transition-all flex items-center justify-center gap-2"
              >
                Buy Credits <ExternalLink className="w-4 h-4" />
              </button>
